...
//QTextBrowser* html = nullptr;

string EasyView::active_ = "cad";
EasyView* EasyView::instanceBase_ = nullptr;
...
//=============================================================================
void EasyView::createTab( const char* id, QWidget* widget, const char* text)
{
    EasyViewTab* tab = new EasyViewTab(this, id, widget, text );
...
void EasyView::onEmptyQueue()
{
    for (int i = 0; i < tabItems_.count(); i++) {
        ...
        QFont font = tab->button_->font();

        if (active_ == tab->id_) {
            //tab->button_->setText(QString(">> ") + tab->text().c_str());
