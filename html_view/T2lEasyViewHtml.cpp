...
//=============================================================================
EasyViewHtml::EasyViewHtml(bool doNotSkip) :
    doNotSkip_(doNotSkip)
{
...
void EasyViewHtml::onEmptyQueue()
{
    ...
    if (doNotSkip_ && active_!=EasyView::active()) {
        setHtml("skipping");
        return;
    }
    ...
    HtmlTab* tab = HtmlTabsRegistry::instance().tabGet_(aaa.c_str());
