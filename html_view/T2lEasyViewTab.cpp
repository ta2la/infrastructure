...
//=============================================================================
EasyViewTab::EasyViewTab(EasyView* parent, const char* id, QWidget* widget, const char* text ) :
    id_(id),
    text_(text),
    parent_(parent)
{
    ...
    button_->move((parent->tabItems_.count())*(SIZE+10), 5);
    button_->resize(SIZE, 30);
    button_->show();
...
//=============================================================================
std::string EasyViewTab::text()
{
    if (text_.empty()) return id_;
    return text_;
}
